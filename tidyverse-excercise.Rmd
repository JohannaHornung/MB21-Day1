...
```{r}
df_vac <- read.csv("COVID-19_Vaccinations_in_the_United_States_County.csv.tar.gz")
```
...
df_vac %>% 
  head() %>% 
  mutate(Date = Date %>% lubridate::mdy()) ## :: means using function from package without loading it
```
...
df_vac_latest %>% 
  summarise(Series_Complete_Pop_Pct)

df_vac_latest %>% 
  pull(Series_Complete_Pop_Pct) %>% # makes it a vector variable
  max(na.rm = TRUE) # because it is a vector variable, I can now calculate the maximum

df_vac_latest %>% 
  filter(Series_Complete_Pop_Pct == max(Series_Complete_Pop_Pct, na.rm = TRUE))
...
df_vac_latest %>% 
  group_by(Recip_State) %>% 
  count(morethan50 = Series_Complete_Pop_Pct > 50) %>% 
  mutate(prop=n/sum(n))
...
df_vac_latest %>% 
  ...
  summarise(totvac = sum(Series_Complete_Yes), mean(Series_Complete_Pop_Pct))
